...
namespace ext {
	...
	// 数学函数指针类型
	//TODO: 低精度回退方案
	typedef long double (*single_param_fp)(long double);
	typedef long double (*double_param_fp)(long double, long double);

	// 基本运算
	long double ceil(acc& val, single_param_fp fp = std::ceil) {
		return fp(val);
	}

	long double floor(acc& val, single_param_fp fp = std::floor) {
		return fp(val);
	}

	long double trunc(acc& val, single_param_fp fp = std::trunc) {
		return fp(val);
	}

	long double round(acc& val, single_param_fp fp = std::round) {
		return fp(val);
	}

	// 三角函数
	long double sin(acc& val, single_param_fp fp = std::sin) {
		return fp(val);
	}

	long double cos(acc& val, single_param_fp fp = std::cos) {
		return fp(val);
	}

	long double tan(acc& val, single_param_fp fp = std::tan) {
		return fp(val);
	}

	long double asin(acc& val, single_param_fp fp = std::asin) {
		return fp(val);
	}

	long double acos(acc& val, single_param_fp fp = std::acos) {
		return fp(val);
	}

	long double atan(acc& val, single_param_fp fp = std::atan) {
		return fp(val);
	}

	// 双曲函数
	long double sinh(acc& val, single_param_fp fp = std::sinh) {
		return fp(val);
	}

	long double cosh(acc& val, single_param_fp fp = std::cosh) {
		return fp(val);
	}

	long double tanh(acc& val, single_param_fp fp = std::tanh) {
		return fp(val);
	}

	long double asinh(acc& val, single_param_fp fp = std::asinh) {
		return fp(val);
	}

	long double acosh(acc& val, single_param_fp fp = std::acosh) {
		return fp(val);
	}

	long double atanh(acc& val, single_param_fp fp = std::atanh) {
		return fp(val);
	}

	// 指数对数函数
	long double exp(acc& val, single_param_fp fp = std::exp) {
		return fp(val);
	}

	long double exp2(acc& val, single_param_fp fp = std::exp2) {
		return fp(val);
	}

	long double expm1(acc& val, single_param_fp fp = std::expm1) {
		return fp(val);
	}

	long double log(acc& val, single_param_fp fp = std::log) {
		return fp(val);
	}

	long double log10(acc& val, single_param_fp fp = std::log10) {
		return fp(val);
	}

	long double log2(acc& val, single_param_fp fp = std::log2) {
		return fp(val);
	}

	long double log1p(acc& val, single_param_fp fp = std::log1p) {
		return fp(val);
	}

	// 其他函数
	long double sqrt(acc& val, single_param_fp fp = std::sqrt) {
		return fp(val);
	}

	long double cbrt(acc& val, single_param_fp fp = std::cbrt) {
		return fp(val);
	}

	long double erf(acc& val, single_param_fp fp = std::erf) {
		return fp(val);
	}

	long double erfc(acc& val, single_param_fp fp = std::erfc) {
		return fp(val);
	}

	long double tgamma(acc& val, single_param_fp fp = std::tgamma) {
		return fp(val);
	}

	long double lgamma(acc& val, single_param_fp fp = std::lgamma) {
		return fp(val);
	}

	// 绝对值（注意：abs 的特殊性）
	long double fabs(acc& val, single_param_fp fp = std::fabs) {
		return fp(val);
	}

	// 幂函数
	long double pow(acc& base, acc& exponent,
	                double_param_fp fp = std::pow) {
		return fp(base, exponent);
	}

	long double hypot(acc& x, acc& y,
	                  double_param_fp fp = std::hypot) {
		return fp(x, y);
	}

	// 浮点数余数
	long double fmod(acc& x, acc& y,
	                 double_param_fp fp = std::fmod) {
		return fp(x, y);
	}

	long double remainder(acc& x, acc& y,
	                      double_param_fp fp = std::remainder) {
		return fp(x, y);
	}

	// 最大值/最小值
	long double fmax(acc& x, acc& y,
	                 double_param_fp fp = std::fmax) {
		return fp(x, y);
	}

	long double fmin(acc& x, acc& y,
	                 double_param_fp fp = std::fmin) {
		return fp(x, y);
	}

	// 浮点数乘加运算 (C++11)
	long double fma(acc& x, acc& y, acc& z,
	                long double (*fp)(long double, long double, long double) = std::fma) {
		return fp(x, y, z);
	}
	...
	// 带符号差值
	long double fdim(acc& x, acc& y,
	                 double_param_fp fp = std::fdim) {
		return fp(x, y);
	}

	// 线性插值 (C++20)
#if __cplusplus >= 202002L
	long double lerp(acc& a, acc& b, acc& t,
	                 long double (*fp)(long double, long double, long double) = std::lerp) {
		return fp(a, b, t);
	}
#endif

	// 注意：这些返回 bool/int，需要特殊处理
	bool isfinite(acc& val, bool (*fp)(long double) = std::isfinite) {
		return fp(val);
	}

	bool isinf(acc& val, bool (*fp)(long double) = std::isinf) {
		return fp(val);
	}

	bool isnan(acc& val, bool (*fp)(long double) = std::isnan) {
		return fp(val);
	}

	bool isnormal(acc& val, bool (*fp)(long double) = std::isnormal) {
		return fp(val);
	}

	int fpclassify(acc& val, int (*fp)(long double) = std::fpclassify) {
		return fp(val);
	}

	bool signbit(acc& val, bool (*fp)(long double) = std::signbit) {
		return fp(val);
	}
